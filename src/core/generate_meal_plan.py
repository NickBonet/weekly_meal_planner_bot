...
def parse_meals_from_mealie_api():
    all_dinner_meals = requests.get(
        os.environ.get("MEALIE_API") + "/api/categories/dinner"
    )
    for meal in all_dinner_meals.json()["recipes"]:
        if "hard" in (string.lower() for string in meal["tags"]):
            hard_recipes.append(
                Recipe(
                    meal["name"].strip(),
                    check_protein_in_tags(meal["tags"]),
                    os.environ.get("MEALIE_API") + "/recipe/" + meal["slug"],
                )
            )
        elif "medium" in (string.lower() for string in meal["tags"]):
            medium_recipes.append(
                Recipe(
                    meal["name"].strip(),
                    check_protein_in_tags(meal["tags"]),
                    os.environ.get("MEALIE_API") + "/recipe/" + meal["slug"],
